...
class HeatSamplingCluster(object):
        ...
        for i, (ratio, radius) in enumerate(zip(self.ratios, self.radii)):
            ...
            if ratio == 1:
                ...
                solver = pp3d.PointCloudHeatSolver(
                    vertices
                )  # bless Nicholas Sharp the G.O.A.T.
                cluster = solver.extend_scalar(
                    indices.tolist(), np.arange(indices.numel())
                )
                ...
                unique, cluster = torch.unique(
                    torch.from_numpy(cluster), return_inverse=True
                )
    ...
    def __repr__(self):
        return "{}(ratios={}, radii={}, loop={}, max_neighbours={})".format(
