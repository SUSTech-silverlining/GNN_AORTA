...
class PointCloudPooling(MessagePassing):
    ...
    def forward(self, x: tuple, pos: tuple, edge_index: torch.Tensor, **kwargs):
        x_i, x_j = x
        pos_i, pos_j = pos
        return self.propagate(edge_index=edge_index, x_i=x_i, x_j=x_j, pos_i=pos_i, pos_j=pos_j, **kwargs)
