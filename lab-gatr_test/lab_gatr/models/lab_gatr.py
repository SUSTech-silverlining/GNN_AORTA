...
class LaBGATr(torch.nn.Module):
    ...
    def __init__(
        ...
        output_channels: int = 1,       # ⭐ 你想要的输出维度（pressure=1）
    ):
        ...
        match pooling_mode:
            case 'message_passing':
                self.tokeniser = Tokeniser(
                    ...
                    d_model,
                    num_latent_channels= 1 * num_latent_channels,
        ...
        self.gatr = gatr.GATr(
            ...
            hidden_s_channels= 1* num_latent_channels,
    ...
    def forward(self, data: Data) -> torch.Tensor:
        ...
        # print("multivectors shape1:", multivectors.shape)#torch.Size([8000, 64, 16])
        ...
        # print("multivectors shape2:", multivectors.shape)#torch.Size([8000, 64, 16])
        # print("sca shape2:", scalars.shape)#torch.Size([8000, 1])

        feats = self.tokeniser.lift(multivectors, scalars)  # 形状应为 [N, 3]（来自 dislodge/extract_translation）
        
        # ❗延迟初始化：第一次调用 forward 时定义 output_head
        if self.output_head is None:
            in_feats = feats.shape[1]
            self.output_head = torch.nn.Linear(in_feats, self.output_channels).to(feats.device)
        out = self.output_head(feats)                       # → [N, output_channels]（如 pressure: [N,1]）
        return out
    ...
    def lift(self, multivectors: torch.Tensor, scalars: torch.Tensor) -> torch.Tensor:
        ...
        if multivectors.size(0) == self.cache['data'].scale0_sampling_index.numel():
            multivectors, scalars = interp(
                ...
                scalars,
                self.cache['scalars'],
                self.cache['pos'], # coarse scale positions 
                self.cache['data'].pos, # fine scale positions 
                self.cache['data'],
                scale_id=0,
                reference_multivector=self.cache['reference_multivector']
            )
...
def interp(
    ...
    # print('scale id: ', scale_id,data[f'scale{scale_id}_interp_source'].shape, data[f'scale{scale_id}_interp_target'].shape, denominator.shape)
    ...
    # print("target max",data[f'scale{scale_id}_interp_target'].max(),data[f'scale{scale_id}_interp_target'] )
    # torch.set_printoptions(threshold=float('inf'))
    # with open("output.txt", "a") as f:  # "a" = append, "w" = overwrite
    #     f.write(
    #         f"target max {data[f'scale{scale_id}_interp_target'].max()} "
    #         f"{data[f'scale{scale_id}_interp_target']}\n"
    #     )
    # print("multivectors shape:", multivectors.shape)
    # print("multivectors_skip shape:", multivectors_skip.shape)
...
class CrossAttentionTokeniser(Tokeniser):
    ...
    def forward(self, data: Data) -> tuple:
        # print("CrossAttentionTokeniser forward called", data)
        multivectors, scalars = self.geometric_algebra_interface.embed(data)
        # print("CrossAttentionTokeniser after called", multivectors.shape, scalars.shape) #torch.Size([16000, 1, 16]) torch.Size([16000, 1])
        ...
        # sample_idx = data.scale0_sampling_index

        # self.cache = {
        #     'multivectors': multivectors[sample_idx],
        #     'scalars': scalars[sample_idx] if scalars is not None else None,
        #     'data': data,
        #     'reference_multivector': self.construct_reference_multivector(multivectors, data.batch)[sample_idx],
        #     'pos': data.pos[sample_idx]
        # }
        ...
        # print("subscale num of points", data.scale0_sampling_index.shape)
        ...
        # print("CrossAttentionTokeniser after cross_attention_hatchling called", multivectors.shape, scalars.shape) # torch.Size([8000, 64, 16]) torch.Size([8000, 1])
